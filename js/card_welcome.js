...
(() => {
  ...
  function showWelcome(location) {
    ...
    else timeChange = "<span>🌌 夜深了，早点休息，明天继续加油！</span>";

    const welcomeInfoElement = document.getElementById("welcome_info");

    if (!welcomeInfoElement) {
      console.error("无法找到欢迎信息元素");
      return;
    }
  ...
  async function onLoad() {
    try {
      if (!cachedLocation) {
        cachedLocation = await getLocationSmart();
      }

      showWelcome(cachedLocation);
    } catch (err) {
      console.error(err);
    }
  }
